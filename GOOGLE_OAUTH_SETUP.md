...
2. Застосуйте міграцію до бази (також після імпорту `sql/init.sql`):

   ```bash
   docker exec -i postgres-db psql -U postgres -d nestdb < sql/migrations/001_google_oauth.sql
   ```

   Або виконайте `npm run db:push`, якщо це ваш звичайний спосіб синхронізації
   Drizzle-схеми.
3. Локальні значення callback мають бути такими:
...
1. Додайте власну схему у `app.json` фронтенду:
...
3. У `.env` бекенду вкажіть:
   ...
   ```env
   OAUTH_SUCCESS_REDIRECT_URL=hitt://auth/google/callback
   ```

   Не додавайте цей `hitt://...` URI у Google Console. У **Authorized redirect
   URIs** має залишитися лише `http://localhost:3000/auth/google/callback`
   (або production HTTPS callback вашого бекенду).
4. Відкривайте авторизацію з RN у браузері, наприклад:

   ```ts
   import * as Linking from 'expo-linking';

   Linking.openURL('https://api.example.com/auth/google');
   ```
   ...
   у Google Console.
5. Обробіть повернення deep link:
